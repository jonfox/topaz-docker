...
docker create --rm --name conf-restore-container -v topaz-conf:/conf topaztechnology/base:3.12.0
...
docker exec conf-restore-container sh -c "echo topaz.kernel-service.external-hostname = ${EXTERNAL_HOSTNAME} >> /conf/system/20-generated.conf"
